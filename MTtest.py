...
from mql5_zmq_backtrader.mt5data import MTraderData
from mql5_zmq_backtrader.mt5store import MTraderStore

StoreCls = MTraderStore
DataCls = MTraderData
...
class TestStrategy(bt.Strategy):
    ...
    def next(self, frompre=False):
        # Visualize data0
        txt = list()
        txt.append(str(self.getdatanames()[0]))
        ...
        if len(self.datas) > 1 and len(self.data1):
            txt = list()
            txt.append(str(self.getdatanames()[1]))
        ...
        if not self.p.trade:
            print('No trading. Parameter trade:', self.p.trade)
            return
...
def runstrategy():
    ...
    storekwargs = dict(
        # ram key_id=args.keyid,
        # ram secret_key=args.secretkey,
        # ram paper=not args.live,
        host="192.168.0.71",
    )

    store = StoreCls(**storekwargs)

    broker = store.getbroker()  # MTraderBroker
    ...
    DataFactory = store.getdata  # MTraderData
...
def parse_args(pargs=None):
    parser = argparse.ArgumentParser(
        ...
        description='Test Metatrader integration')
    ...
    parser.add_argument('--stake', default=0.1, type=float,
                        required=False, action='store',
...
if __name__ == '__main__':
    start_date = datetime.datetime.now() - datetime.timedelta(minutes=500)
    sys.argv = [
        'MTtest2.py',
        '--data0', 'BTCEUR',
        '--timeframe', 'Minutes',
